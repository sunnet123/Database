...
GRANT ALL PRIVILEGES ON `Bank`.* TO 'sunnet123'@'%';
GRANT ALL PRIVILEGES ON `Shop`.* TO 'sunnet123'@'%';
FLUSH PRIVILEGES;
...
#다른방법
SELECT * FROM `bank_account`
WHERE `a_balance`=(SELECT MAX(`a_balance`) FROM `bank_account` WHERE `a_item_name`='자유저축예금');


#실습2-19
SELECT * FROM `bank_transaction` WHERE `t_amount` != 0
ORDER BY `t_dist`, `t_amount` DESC;

#실습2-20
SELECT
	COUNT(IF(`t_dist`=1,1,NULL)) AS `입금 건수`,
    COUNT(IF(`t_dist`=2,2,NULL)) AS `출금 건수`,
    COUNT(IF(`t_dist`=3,3,NULL)) AS `조회 건수`
FROM `bank_transaction`;
