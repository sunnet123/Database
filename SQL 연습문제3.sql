...
GRANT ALL PRIVILEGES ON `College`.* TO 'sunnet123'@'%';
FLUSH privileges;
...
JOIN `Register` AS b ON a.`stdNo` = b.`regStdNo`
JOIN `Lecture` AS c ON b.`regLecNo` = c.`lecNo`
ORDER BY b.`regGrade` ASC, `regFinalScore` DESC; 
...
JOIN `Register` AS b ON a.`stdNo` = b.`regStdNo`
JOIN `Lecture` AS c ON b.`regLecNo` = c.`lecNo`
WHERE b.`regTotalScore` >= 60 GROUP BY a.`stdNo`;